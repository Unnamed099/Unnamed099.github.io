...
function renderSingleRow(row) {
    ...
    div.style.left = (row.x || 0) + 'px';
    div.style.top = (row.y || 0) + 'px';
    div.style.cursor = 'grab';
    div.style.userSelect = 'none';
    div.style.touchAction = 'none';

    const dragHandle = document.createElement('div');
    dragHandle.style.cssText = 'display:flex;justify-content:space-between;align-items:center;gap:6px;cursor:move;';
    ...
    if (row.type === 'latex') {
        ...
        canvas.style.cssText = 'border:1px solid #ddd;border-radius:4px;cursor:crosshair;background:transparent;display:block;';
    ...
    div.addEventListener('mousedown', (e) => {
        if (e.target.tagName === 'SELECT' || e.target.tagName === 'BUTTON' || e.target.tagName === 'INPUT' || e.target.tagName === 'CANVAS' || e.target.tagName === 'IMG') return;
        dragRow = row;
        dragRowElement = div;
        dragOffsetX = e.clientX - (row.x || 0);
        dragOffsetY = e.clientY - (row.y || 0);
        e.preventDefault();
    });
    ...
    div.addEventListener('touchstart', (e) => {
        ...
        dragRow = row;
        dragRowElement = div;
        dragOffsetX = touch.clientX - (row.x || 0);
        dragOffsetY = touch.clientY - (row.y || 0);
    }, { passive: false });
...
document.addEventListener('mousemove', (e) => {
    if (!dragRow || !dragRowElement) return;
    dragRow.x = e.clientX - dragOffsetX;
    dragRow.y = e.clientY - dragOffsetY;
    dragRowElement.style.left = dragRow.x + 'px';
    dragRowElement.style.top = dragRow.y + 'px';
});
...
document.addEventListener('touchmove', (e) => {
    if (!dragRow || !dragRowElement) return;
    const touch = e.touches[0];
    dragRow.x = touch.clientX - dragOffsetX;
    dragRow.y = touch.clientY - dragOffsetY;
    dragRowElement.style.left = dragRow.x + 'px';
    dragRowElement.style.top = dragRow.y + 'px';
    e.preventDefault();
}, { passive: false });
    ...
    filtered.forEach(s => {
        ...
        btn.addEventListener('click', () => {
            if (activeLatexInput) {
                ...
                activeLatexInput.selectionStart = activeLatexInput.selectionEnd = start + s.latex.length;
